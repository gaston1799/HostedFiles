...
local getPathToPosition = pathfinding.getPathToPosition
local moveToTarget = pathfinding.moveToTarget
--Miners haven
local catalystData = loadstring(game:HttpGet(base.."items.lua"))()
...
local setupPages = require(script.modules.ui_pages)
local pages = setupPages(UI)

local Main = pages.Main
local Miners = pages.Miners
local Legends = pages.Legends
local BloxF = pages.BloxF
local Gameplaydiv = pages.Gameplaydiv
local Orbs = pages.Orbs
local Minersdiv = pages.Minersdiv
local Fruits = pages.Fruits
...
Minersdiv:addButton(
    {
        ...
        callback = function()
            ...
            local obj = catalystData.Catalysts
            local Fusions = catalystData.Fusions
            local Evolved = catalystData.Evolved
            function getItem(name)
                return game.ReplicatedStorage.Items[name]
            end
